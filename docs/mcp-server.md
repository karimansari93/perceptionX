...
- **Frontend** — `/chat` is `ProtectedRoute` + `AskAiRoute` (any owner/admin/member of an organization; `VITE_ASK_AI_ENABLED=false` is the platform-wide kill switch that also hides the sidebar "Ask AI" entry (Analyze group) and the overview chat box). Answers render with `react-markdown` + `remark-gfm`; links open the exact returned URL in a new tab as favicon badges, competitor names the tools returned this turn (`{competitors}` SSE event) carry their logos, non-http(s) hrefs render as plain text. The overview tab opens with the chat box ("Hey <first name>, what do you want to do today?"); a question typed there opens `/chat` with it already sent.
- **Question scope** — the dashboard's filters (company, market, job function) travel with a question: the overview chat box says what it is asking about and hands the scope to `/chat`; the chat opens with a scope step (company, then multi-select markets and multi-select job functions, fed by `action: "scope"`; several values mean one filtered tool call each, compared side by side) and keeps the same pickers above the input ( — the organization's brands and one brand's tracked markets and job functions, the same spellings the tools match) and scope chips on every question. The scope goes to `chat-with-data` as `body.scope` and is appended to the user turn as a second text block naming the `location` / `job_function` filters to pass (`chat-with-data/scope.ts`); brand-wide questions carry no note, and the system prompt (the cached prefix) is untouched. The job-function filter itself now lives in the dashboard's top bar (`JobFunctionFilter`, next to Period and Location) instead of per-tab pill rows.
- **Design (2026-09-07, "PerceptionX Chat" handoff)** — three screens recreated in the codebase's components: the overview hero (company logo, ASK ABOUT pill chips, 46px composer, teal starter pills) with the one-card spec for EPS and Breakdown and a one-screen layout at 1080×720 and above; the Ask AI page (centred greeting, composer card with scope chips — company, markets, functions; the period chip and the starters were dropped on 2026-09-08 so the composer is the focus — and the three most recent chats with their scope); and the Answer page (thread bar, SCOPE row from the thread's scope plus the `px-context` block, 15px prose, metric cards, contribution chart, delta-coloured comparison tables with competitor logos and semantic "vs brand" colouring, source pills with response counts, `px-followups` pills, pinned follow-up composer). Threads snapshot their scope in `chat_conversations.scope` (migration `20260907150000_chat_conversation_scope.sql`, applied) so a reopened chat shows the scope it was answered in. The chats list (Chat · scope · updated) is reached from "All chats". Not built, per the handoff: the side panel / dock entry points and the returning-user announcement.
- **Voice (2026-09-08, chat-with-data v39)** — the "How to respond" section of the prompt asks for chat prose, not a report: answer first in plain words, numbers woven into two-to-four-sentence paragraphs, bullets only for parallel items, a heading only when an answer has distinct parts, visual blocks used when they carry the numbers better than a sentence (not as a template), a natural close. Process narration ("I'll pull the company list first") is banned in the prompt and held back in the stream: each round's first 200 characters of text (`HOLD_CHARS`) are buffered and dropped if a tool call starts in that round, flushed once the text is clearly an answer or the round ends, so the reader sees the tool status line and then the answer. Verified live on Netflix: status line only while tools run, then "Straight answer: …".
- **Logos** — every company and source the analyst names gets a logo.dev mark, in prose and table cells alike: the competitors the tools returned plus the brand itself (`px-competitor:` links → `getCompetitorFavicon`), bare domains such as glassdoor.com or jobs.netflix.com (`px-domain:` → `getFavicon`) and well-known sources named in words (Glassdoor, Reddit, LinkedIn, Indeed, Blind, Comparably, Levels.fyi, Wikipedia, YouTube… plus the first label of every domain behind that answer). The decoration runs on the finished answer only (`decorateEntities` in `ChatMessage.tsx`) and skips code, existing links and URLs; react-markdown's URL transform lets the two `px-` schemes through. logo.dev answers 401 without `VITE_LOGO_DEV_TOKEN`, so an environment without it (this dev machine) shows letter marks everywhere the dashboard does.
- **What's-new modal + "Set up AI integrations" card (2026-09-08, spec CHANGES-announcement-and-integrations)** — `WhatsNewModal` (three steps in one card: the announcement with the looping Ask AI demo, the "which assistants does your team use?" form, the confirmation) opens once per announcement version for returning users (accounts created before `ANNOUNCEMENT_RELEASED_AT` in `src/lib/announcements.ts`; bump `ANNOUNCEMENT_VERSION` to ship the next one). Every way out — ✕, scrim, Esc, Try Ask AI, finishing the form — writes `announcement_seen (user_id, version)` (migration `20260908120000_announcements_and_integration_requests.sql`, applied). The sidebar footer card above "Want a guided tour?" reopens it straight on the form; the form calls the `integration-request` edge function, which inserts `integration_requests (user_id, organization_id, tools[], other, note, reply_to)` and then posts the request to Slack (`INTEGRATION_REQUESTS_SLACK_WEBHOOK`, else the invite-alerts webhook, best-effort after the row is stored) — admins read the table (`select * from integration_requests order by created_at desc`). Both hide behind `VITE_ASK_AI_ENABLED`. Verified live as a CSL Behring member: auto-open, chips + validation, a stored request (deleted after the test), no re-open after dismissal, the card opens on the form.
- **Answer persistence (2026-09-08, chat-with-data v42)** — the finished answer is saved to the thread by the function (`saveAssistantMessage`: the conversation must belong to the caller; sources stored alongside), not by the browser, and a client that disconnects mid-stream (thread switch, navigation, closed tab, Stop) no longer aborts the analyst — it finishes and saves, so the thread has the answer when the user comes back. The browser cancels its on-screen stream when a thread is opened or the page unmounts, so nothing bleeds into another thread. Stop therefore means "stop watching": the full answer still lands in the thread.
- **Laptop heights (2026-09-08)** — Prompts sits under Analyze (no Monitoring group). The sidebar footer cards compact by viewport height: sub-lines hide below 860px, the tour and invite cards below 740px, the integrations card below 660px, and the groups sit tighter, so the nav never scrolls (checked at 1366×768, 720 and 660). The overview hero has no suggestion pills; the three summary cards use the EPS card header placement, a smaller View All and tighter rows, so five rows fit without an inner scrollbar at 1366×768.
...
Posts the manual QA script from `docs/ask-ai-build-brief.md` §5 plus every `questions.json` question to `chat-with-data`, collects each stream, prints the answers verbatim, and lints them: no calendar-gap language ("missing", "gap", "no data for May", "still filling in", "pause"), no decimal sentiment or visibility, no other-customer talk, every markdown link's host present in that turn's `{sources}` event and every link URL one the tools returned (a bare domain link fails), and every answer that names a source carrying at least one link. Reports time-to-first-token p50 (the brief's rule: drop `CLAUDE_EFFORT` to `medium` if it sits above ~6 s on Ford). `--only "<substring>"` narrows the set, `--extra "<question>"` adds one. Run it on Ford and PepsiCo after any change to the prompt, the rulebook or the tool layer. **Run record:** 2026-09-07, chat-with-data v33 (v34 adds the question scope) — the §5 script was run by hand in the browser as a Netflix owner (the session had no Ford or PepsiCo login): 7/7 pass — Q3 2026 scorecard in percentages and points vs Q2 2026, wellbeing change with the sources in play and their pages, ten Glassdoor pages linked to the exact returned URLs, a by_job_function visibility split, an org-only refusal for "other PerceptionX customers", "May was never a separate measurement period", and an engineering job description drawn from the Engineering-filtered themes with the AI-answers caveat. Eight logged requests: time-to-first-token p50 2.1 s, duration p50 31 s, 239k cache-read vs 128k uncached input tokens, so `CLAUDE_EFFORT` stays `high`. `chat-eval.ts` itself has not yet been run with credentials — the first scripted run on Ford and PepsiCo belongs here.

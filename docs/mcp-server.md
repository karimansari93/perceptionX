...
- **What's-new modal + "Set up AI integrations" card (2026-09-08, spec CHANGES-announcement-and-integrations)** — `WhatsNewModal` (three steps in one card: the announcement with the looping Ask AI demo, the "which assistants does your team use?" form, the confirmation) opens once per announcement version for returning users (accounts created before `ANNOUNCEMENT_RELEASED_AT` in `src/lib/announcements.ts`; bump `ANNOUNCEMENT_VERSION` to ship the next one). Every way out — ✕, scrim, Esc, Try Ask AI, finishing the form — writes `announcement_seen (user_id, version)` (migration `20260908120000_announcements_and_integration_requests.sql`, applied). The sidebar footer card above "Want a guided tour?" reopens it straight on the form; the form inserts `integration_requests (user_id, organization_id, tools[], other, note, reply_to)` — users insert their own, admins read them (`select * from integration_requests order by created_at desc`). Both hide behind `VITE_ASK_AI_ENABLED`. Verified live as a CSL Behring member: auto-open, chips + validation, a stored request (deleted after the test), no re-open after dismissal, the card opens on the form.

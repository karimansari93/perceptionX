...
**State as of 2026-09-03 (Ford pilot prep):** the migrations above are applied (plus `20260903130000_ai_themes_company_attribute_index.sql` and `20260903140000_mcp_attribute_sources_sampled.sql`, found during the live eval: the attribute-sources read took 15 s on Ford's brand scope and the edge function's PostgREST connection stops statements at 8 s — it now samples up to 2,000 answers and returns in ~0.1 s) and the Ford org is enabled. `mcp-server` v10 is deployed as a single minified bundle (`functions/mcp-server/index.js`, built with `deno bundle --platform deno --minify --external 'https://deno.land/*' --external 'https://esm.sh/*' mcp-server/index.ts`) because the session had no Supabase CLI; the live eval passes 111/111 against it with Ford data. The next `supabase functions deploy mcp-server --project-ref ofyjvfmcgtntwamkubui` from this branch replaces the bundle with the source files (identical behavior). `chat-with-data` (the admin-only in-app chat) still runs the pre-audit code and needs `supabase functions deploy chat-with-data` from this branch.
...
- **Theme↔citation linkage is response-level only, and sampled.** `sources_in_attribute_answers` (on a focused `get_attribute_themes`) lists the domains cited in a random sample of up to 2,000 of the latest period's answers that carry a theme of that attribute — "the sources in play when wellbeing comes up" — with `sample_size.answers_sampled` stating the denominator. Exploding every answer's citation JSON at request time exceeds the 8 s PostgREST statement budget on a brand-wide scope; a per-attribute domain cube would make it exact. Which citation produced which sentence would need extraction-time changes; the payload says "association, not cause" for that reason.

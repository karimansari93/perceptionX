...
// ---------------------------------------------------------------------------
// Google redirect / wrapper URLs (adopted from the 2026-08-04 hotfix bundle of
// test-prompt-google-ai-mode, which was deployed without a matching commit)
//
 ...
 * strips any #:~:text= text-highlight fragment Google appends. Wrappers
 * occasionally nest (an image redirect around a translate link), so
 * unwrapping repeats until the URL stops changing. Superset of
 * unwrapTranslateUrl above, which is kept for existing callers.
 *
...
export function normalizeCitationsForStorage<T extends { url?: string; domain?: string; title?: string }>(
  ...
  for (const citation of citations) {
    ...
    // so dedupe on the destination, not on what the payload handed us.

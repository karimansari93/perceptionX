...
import { fetchGoogleAiMode } from "../_shared/google-serp.ts"
...
// never abort on a missing AI Mode answer.
...
serve(async (req) => {
  ...
  try {
    ...
    const result = await fetchGoogleAiMode(prompt, location_context)
    return new Response(

...
// Primary model: `gpt-5.6-sol`, the model paid ChatGPT users (Plus, Pro) get
// by default since the GPT-5.6 rollout (July 2026). For GEO measurement,
// matching the live ChatGPT model is the point — citations only mean something
// if they reflect what real users see.
...
const MODEL_FALLBACKS = ['gpt-5.5', 'gpt-5.2']
...
async function callOpenAIWebSearch(prompt: string, useWebSearch: boolean, modelOverride?: string): Promise<{
  ...
  const callOnce = async (model: string, serviceTier?: string) => {
    const body: Record<string, any> = {
      ...
      // the reasoning (gpt-5.x) models accept this parameter. GPT-5.6 dropped
      // "minimal" in favour of "none".
      ...(model.startsWith('gpt-5')
        ? { reasoning: { effort: useWebSearch ? 'low' : (model.startsWith('gpt-5.6') ? 'none' : 'minimal') } }
        : {}),

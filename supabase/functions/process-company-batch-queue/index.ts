...
interface TickPayload {
  /** Scope every claim + chain to one config (Start Collection, watchdog kick). */
  configId?: string;
  promptTypes?: string[];
  models?: string[];
  /**
   * True when this invocation was spawned by another worker (self-chain or
   * fan-out). Only *root* kicks fan out, so worker count per config stays
   * bounded at MAX_PARALLEL_WORKERS instead of doubling every generation.
   */
  chained?: boolean;
}

/** Max workers a single root kick will put on one config (one per queue row). */
const MAX_PARALLEL_WORKERS = 3;

/** How many pending rows to try claiming before giving up on this invocation. */
const CLAIM_CANDIDATES = 5;

/**
 * Fire another invocation of this function and wait only for the *response
 * headers* — which now arrive in milliseconds, because the handler answers 202
 * immediately and does its work in the background (see `serve` below).
 *
 * This is the whole reason the chain used to die. Previously the callee only
 * responded once its chunk was finished (~2 min), and the caller was pinned
 * alive by EdgeRuntime.waitUntil for that entire time — so every generation of
 * the chain kept its whole ancestry resident. When the root eventually hit the
 * runtime's wall-clock limit it was torn down, its socket to the child closed,
 * and the whole chain collapsed with it. Collection then sat idle until the
 * 5-minute watchdog kicked a fresh root — which is exactly the "reset stranded
 * jobs" alert loop.
 */
async function kickWorker(
  ...
  try {
    const res = await fetch(`${supabaseUrl}/functions/v1/process-company-batch-queue`, {
      method: "POST",
      headers: {
    ...
    // Release the socket; we never care about the body.
    await res.body?.cancel().catch(() => {});
  } catch (e) {
    console.error("[BatchQueue] Failed to kick worker:", e);
  }
}

async function processQueueTick(payload: TickPayload) {
  ...
  const { configId, promptTypes, models, chained } = payload;

  /** Body every worker we spawn inherits, so scope/model choices survive the chain. */
  const chainBody: TickPayload = {
    ...(configId ? { configId } : {}),
  ...
  // -----------------------------------------------------------------------
  // Atomic claim:
  //   1. SELECT the oldest 'pending' candidates (NOT 'processing' — we never
  //      resume another worker's in-flight row; that's the watchdog's job).
  //   2. UPDATE WHERE status='pending' as a compare-and-swap — only one
  //      worker can flip a given row to 'processing'.
  //
  // We walk several candidates rather than one: with more than one worker on
  // a config, a single-candidate claim meant every worker raced for the same
  // (oldest) row and all but one exited immediately. That's why sibling jobs
  // starved — CSL Limited sat pending for a whole day while CSL Behring was
  // the only row anyone ever tried to claim. Walking the list means N workers
  // land on N distinct rows.
  //
  ...
  // succeed on different rows or one wins and the other moves on.
  ...
  for (const candidate of candidates) {
    ...
    // Lost this row to another worker — try the next candidate.
  }

  if (!claimed) {
    // Every candidate is already owned. Exit cleanly — those workers will
    // chain onwards; adding another here would just spin.
    return { processed: 0, message: "Lost claim race — all candidates already claimed" };
  }
  ...
  // -----------------------------------------------------------------------
  // Fan out. A single lineage works exactly one row at a time, so a config
  // with several queue rows finishes them strictly one after another — the
  // sibling rows just sit pending, look stranded to the watchdog, and get
  // reset (and alerted) every 5 minutes for as long as the run lasts.
  //
  // Only a *root* kick fans out (cron watchdog, admin UI). Every worker it
  // spawns is marked `chained`, so the count per config is capped at
  // MAX_PARALLEL_WORKERS instead of doubling with each generation.
  // -----------------------------------------------------------------------
  if (!chained) {
    ...
    if (configId) siblingQuery = siblingQuery.eq("config_id", configId);

    const { count: pendingSiblings } = await siblingQuery;
    const extraWorkers = Math.min(pendingSiblings || 0, MAX_PARALLEL_WORKERS - 1);

    if (extraWorkers > 0) {
      console.log(`[BatchQueue] Fanning out ${extraWorkers} extra worker(s) for ${configId ? `config ${configId}` : "global queue"}`);
      await Promise.all(
        Array.from({ length: extraWorkers }, () => kickWorker(supabaseUrl, supabaseKey, chainBody)),
      );
    }
  }
  ...
  try {
    ...
    if (count && count > 0) {
      ...
      // Awaited, not fire-and-forget: the successor answers 202 in
      // milliseconds, so this returns as soon as the request is on the wire.
      await kickWorker(supabaseUrl, supabaseKey, chainBody);
    }
  } catch (err: any) {
    ...
    // Chain on regardless of whether this row survived: a permanently failed
    // row must not take the rest of the config's queue down with it. Back off
    // briefly first so a fast-failing dependency can't spin the chain.
...
serve(async (req) => {
  ...
  const payload: TickPayload = await req.json().catch(() => ({}));

  const work = processQueueTick(payload).catch((error: any) => {
    console.error("[BatchQueue] Fatal error:", error);
    return { processed: 0, message: `Fatal: ${error?.message}` };
  });

  // Answer immediately and keep working in the background. Callers of this
  // function — the pg_cron watchdog (5s pg_net timeout), the admin UI, and
  // every chained worker — only need to know the tick was accepted. Holding
  // the response until the chunk finished is what made the self-chain pin its
  // whole ancestry in memory and collapse a few minutes later.
  ...
  // No background-task support (e.g. local `supabase functions serve`) — fall
  // back to running inline so behaviour stays correct in development.

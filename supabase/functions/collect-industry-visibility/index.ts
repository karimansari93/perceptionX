...
import { SOURCES_SECTION_REGEX, normalizeCitationsForStorage } from "../_shared/citation-extraction.ts";
 ...
 * on every citation so the stored prompt_responses.citations never contains a
 * google.com redirect, and drop wrappers with no recoverable destination.
 * Re-derives `domain` from the unwrapped URL so analytics aggregates correctly.
 */
function unwrapCitations(citations: any[]): any[] {
  return normalizeCitationsForStorage(citations);
}

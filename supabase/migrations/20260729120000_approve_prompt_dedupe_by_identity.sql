-- Approve-time prompts are now localized per market (translate-prompts) before
-- insertion. LLM translation is not deterministic, so re-approving a brief
-- would re-insert differently-worded translations of cells that already have
-- their rows if dedupe stayed text-only. Dedupe on the prompt's logical
-- identity — attribute × type × function × market × industry — when the row
-- carries one, keeping the text match as the guard for legacy rows without
-- attribute_id. Everything else in admin_approve_intake is unchanged from
-- 20260705140000_methodology_v2_db.sql.
...
begin
  ...
  -- Dedupe on logical identity (attribute × type × function × market ×
  -- industry) when the row carries an attribute_id — approved rows are
  -- localized, and translation wording is not reproducible — falling back to
  -- case-insensitive text for legacy rows without one.
  for v_row in select * from jsonb_array_elements(p_rows)
  loop
    v_company_id := (v_companies->>(v_row->>'entity_name'))::uuid;
    if exists (
      select 1 from confirmed_prompts cp
      where cp.company_id = v_company_id
        and (
          (
            nullif(v_row->>'attribute_id', '') is not null
            and cp.attribute_id = v_row->>'attribute_id'
            and cp.prompt_type = v_row->>'prompt_type'
            and coalesce(cp.job_function_context, '') = coalesce(v_row->>'job_function_context', '')
            and coalesce(cp.location_context, '') = coalesce(v_row->>'location_context', '')
            and coalesce(cp.industry_context, '') = coalesce(v_row->>'industry_context', '')
          )
          or lower(cp.prompt_text) = lower(v_row->>'prompt_text')
        )
    ) then

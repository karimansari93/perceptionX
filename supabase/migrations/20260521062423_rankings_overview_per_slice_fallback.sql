...
-- Applied to production as migration version 20260521062423; this file was
...
WITH source AS (
  SELECT pr.id AS response_id, pr.ai_model, pr.index_period,
...
slice_latest AS (
  SELECT industry_context, country,
         CASE WHEN bool_or(index_period IS NULL) THEN NULL::text
              ELSE max(index_period) END AS data_period
  FROM source
  GROUP BY industry_context, country
),
raw_data AS (
  SELECT s.response_id, s.ai_model, s.industry_context, s.country, s.prompt_theme,
         s.detected_competitors, sl.data_period
  FROM source s
  JOIN slice_latest sl USING (industry_context, country)
  WHERE s.index_period IS NOT DISTINCT FROM sl.data_period
),
industry_stats AS (
  SELECT industry_context, country, data_period,
         array_agg(DISTINCT (ai_model || '::' || COALESCE(prompt_theme,'null'))) AS all_industry_combinations
  FROM raw_data
  GROUP BY industry_context, country, data_period
),
raw_split AS (
...
canonical_aggregated AS (
  SELECT canonical_name, industry_context, country, data_period,
  ...
  FROM mapped
  GROUP BY canonical_name, industry_context, country, data_period
),
scored AS (
  ...
  JOIN industry_stats ist USING (industry_context, country, data_period)

...
-- Applied to production as migration version 20260520134834; this file was
...
SELECT canonical_name, website_domain, industry_context, country,
...
WITH raw_data AS (
  SELECT pr.id AS response_id, pr.index_period, pr.ai_model,
         cp.industry_context, cp.location_context AS country, cp.prompt_theme,
         pr.detected_competitors
  FROM prompt_responses pr
  JOIN confirmed_prompts cp ON pr.confirmed_prompt_id = cp.id
...
raw_split AS (
  SELECT rd.response_id, rd.index_period, rd.ai_model, rd.industry_context, rd.country, rd.prompt_theme,
         clean_company_name(token.token) AS company_name
  FROM raw_data rd
  CROSS JOIN LATERAL regexp_split_to_table(rd.detected_competitors, '[,;\n]+') token(token)
  WHERE length(TRIM(BOTH FROM token.token)) > 1
...
mapped AS (
  SELECT DISTINCT
         rs.response_id, rs.index_period, rs.ai_model, rs.industry_context, rs.country, rs.prompt_theme,
         cm.canonical_name, cm.website_domain
  FROM raw_split rs
  JOIN canonical_map cm ON cm.raw_name = rs.company_name
),
canonical_aggregated AS (
  SELECT index_period, canonical_name, industry_context, country,
         max(website_domain) AS website_domain,
         count(DISTINCT response_id) AS mention_count,
         array_agg(DISTINCT (ai_model || '::' || COALESCE(prompt_theme,'null'))) AS combinations
  FROM mapped
...
scored AS (
  SELECT ca.index_period, ca.canonical_name, ca.website_domain, ca.industry_context, ca.country,
         ca.mention_count, ca.combinations, pis.all_combinations AS all_period_combinations,
         COALESCE(round(LEAST(100.0,
           (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c))
           / NULLIF((SELECT count(*)::numeric FROM unnest(pis.all_combinations) c(c)), 0::numeric)
           * 100::numeric), 1), 0::numeric) AS score_all,
         COALESCE(round(LEAST(100.0,
           (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c) WHERE c.c ILIKE '%gpt%')
           / NULLIF((SELECT count(*)::numeric FROM unnest(pis.all_combinations) c(c) WHERE c.c ILIKE '%gpt%'), 0::numeric)
           * 100::numeric), 1), 0::numeric) AS score_chatgpt,
         COALESCE(round(LEAST(100.0,
           (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c) WHERE c.c ILIKE '%google%' OR c.c ILIKE '%gemini%' OR c.c ILIKE '%bard%')
           / NULLIF((SELECT count(*)::numeric FROM unnest(pis.all_combinations) c(c) WHERE c.c ILIKE '%google%' OR c.c ILIKE '%gemini%' OR c.c ILIKE '%bard%'), 0::numeric)
           * 100::numeric), 1), 0::numeric) AS score_google,
         COALESCE(round(LEAST(100.0,
           (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c) WHERE c.c ILIKE '%perplexity%')
           / NULLIF((SELECT count(*)::numeric FROM unnest(pis.all_combinations) c(c) WHERE c.c ILIKE '%perplexity%'), 0::numeric)
           * 100::numeric), 1), 0::numeric) AS score_perplexity
  FROM canonical_aggregated ca
  JOIN period_industry_stats pis USING (index_period, industry_context, country)
  WHERE EXISTS (SELECT 1 FROM company_employee_tiers cet WHERE lower(cet.company_name)=lower(ca.canonical_name) AND cet.estimated_tier IS NOT NULL AND cet.estimated_tier <> ALL (ARRAY['<50','50-499','500-4999']))
    AND NOT EXISTS (SELECT 1 FROM company_overrides co WHERE lower(co.canonical_name)=lower(ca.canonical_name) AND co.status='excluded')
    AND NOT EXISTS (SELECT 1 FROM company_overrides co WHERE lower(co.canonical_name)=lower(ca.canonical_name) AND co.allowed_industries IS NOT NULL AND ca.industry_context <> ALL (co.allowed_industries))
)
SELECT index_period, canonical_name, website_domain, industry_context, country,
       mention_count, combinations, all_period_combinations,
       score_all, score_chatgpt, score_google, score_perplexity
FROM scored;

CREATE UNIQUE INDEX rankings_historical_uk            ON public.rankings_historical (index_period, canonical_name, industry_context, country);
CREATE UNIQUE INDEX idx_rankings_historical_unique    ON public.rankings_historical (index_period, canonical_name, industry_context, country);
CREATE        INDEX idx_rankings_historical_canonical ON public.rankings_historical (canonical_name);
CREATE        INDEX idx_rankings_historical_industry_country_period ON public.rankings_historical (industry_context, country, index_period);

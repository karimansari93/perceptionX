...
-- Applied to production as migration version 20260807130417; this file was
-- back-filled afterwards and therefore post-dates the deployment.

-- Fix slug resolution for companies whose TS-normalized snapshot name does
-- not literally exist in the rankings matview (e.g. snapshot "Bain" vs
-- matview "Bain & Company"). api_resolve_slug returned the normalized name,
-- api_company found no current row under it, and fell through to the
-- historical branch (or 404) — /company/bain served a "last appeared" page
-- for the #1 US Consulting firm. ~24 snapshots were in this class.
--
-- company_snapshots.source_name now records the matview canonical_name the
-- snapshot's rank row came from; api_resolve_slug prefers it, and
-- api_company's summary lookup matches the snapshot by requested slug.

ALTER TABLE public.company_snapshots ADD COLUMN IF NOT EXISTS source_name text;
...
AS $function$
  SELECT coalesce(
    ...
    (SELECT coalesce(nullif(source_name, ''), canonical_name)
     FROM company_snapshots WHERE slug = lower(trim(p_slug)) LIMIT 1),
...
-- api_company: the stored-summary lookup now matches the snapshot row by the
-- requested slug first (the row that generated this page), falling back to
-- canonical_name for API callers that resolved another way.
...
BEGIN
  ...
  WHERE slug = lower(trim(p_slug)) OR lower(canonical_name) = lower(v_canonical)
  ORDER BY (slug = lower(trim(p_slug))) DESC
  LIMIT 1;

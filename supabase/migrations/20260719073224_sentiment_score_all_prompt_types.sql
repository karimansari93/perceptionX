...
-- Applied to production as migration version 20260719073224; this file was
-- back-filled afterwards and therefore post-dates the deployment.

-- Methodology fix (2026-07-18, user-approved): the dashboard sentiment score
-- previously counted only themes from prompts of type 'sentiment'/'competitive'.
-- The prompt library uses informational/experience/discovery/competitive (no
-- 'sentiment' type), so ~70% of themes — including the heavily-negative
-- 'experience' prompts — were excluded from the headline score while the
-- attribute-themes exhibits counted everything. From this migration on, the
-- sentiment score counts ALL themes, matching the attribute table. Applied at
-- the July-2026 fresh-baseline cycle so no quarter-over-quarter restatement is
-- needed.
...
--    Created WITH NO DATA (population runs server-side immediately after via a
--    one-shot job — the full build exceeds interactive statement budgets).

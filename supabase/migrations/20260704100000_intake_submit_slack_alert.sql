-- Notify the team in Slack when a client submits their onboarding brief.
-- Reuses the existing alert pipeline: send_batch_alert() → send-batch-alert
-- edge function → BATCH_ALERTS_SLACK_WEBHOOK. Fire-and-forget: a Slack outage
-- (or unconfigured webhook) must never fail a client's submission.
...
begin
  ...
  -- Slack heads-up for the team (best-effort, never blocks the submission)
  begin
    perform public.send_batch_alert(jsonb_build_object(
      'event', 'custom',
      'title', '📥 Onboarding brief submitted',
      'text', '*' || v_invite.company_name || '* completed their onboarding brief. Review it in Admin → Intake Invites.',
      'fields', jsonb_build_array(
        jsonb_build_object('label', 'Company', 'value', v_invite.company_name),
        jsonb_build_object('label', 'Contact', 'value', v_invite.contact_email),
        jsonb_build_object(
          'label', 'Functions',
          'value', coalesce(
            (select string_agg(x, ', ') from jsonb_array_elements_text(v_payload->'job_functions') x), '—')
        ),
        jsonb_build_object(
          'label', 'Markets',
          'value', coalesce(
            (select string_agg(x, ', ') from jsonb_array_elements_text(v_payload->'markets') x), '—')
        )
      )
    ));
  exception when others then
    raise notice 'intake_submit: slack alert failed: %', sqlerrm;
  end;

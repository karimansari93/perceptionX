-- Fold per-response sentiment into the response stream. The dashboard used to
-- page company_response_sentiment_mv separately (measured: 92 requests /
-- 4.5 MB per Ford-scope load — the last remaining per-company fan-out). The
-- MV is response-grain with a unique (company_id, response_id) index, so a
-- LEFT JOIN adds one index probe per streamed row and the separate family
-- disappears entirely.

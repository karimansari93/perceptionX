...
-- Applied to production as migration version 20260811111115; this file was
-- back-filled afterwards and therefore post-dates the deployment.

-- Final shape; mirrors supabase/migrations/20260811160000_analysis_health_by_org.sql
...
BEGIN
  ...
  SELECT o.id,
         ...
         od.dirty_since,
         CASE
           -- Some orgs have responses that predate collection_cycle entirely;
           -- last_response_at is still populated for them, so this is "no cycle
           -- to judge", not "no data at all".
           WHEN (v_org_cycle -> o.id::text) IS NULL THEN 'no_cycle'
...
REVOKE ALL ON FUNCTION public.get_analysis_health_by_org() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_analysis_health_by_org() TO authenticated, service_role;

COMMENT ON FUNCTION public.get_analysis_health_by_org() IS
  'Admin-only. One readiness row per organization, each asked against that org''s own latest collection cycle. entity_suggestions_affecting is the subset of platform-wide pending entity merges that touch the org''s competitor set.';

...
-- Applied to production as migration version 20260522133449; this file was
...
WITH raw_data AS (
  ...
  WHERE pr.for_index = true AND pr.index_period IS NULL
    AND cp.industry_context IS NOT NULL AND cp.location_context IS NOT NULL
),
industry_stats AS (
  SELECT industry_context, country,
         array_agg(DISTINCT (ai_model || '::' || COALESCE(prompt_theme,'null'))) AS all_industry_combinations
  FROM raw_data GROUP BY industry_context, country
),
raw_split AS (
...
mapped AS (
  SELECT DISTINCT rs.response_id, rs.ai_model, rs.industry_context, rs.country, rs.prompt_theme,
         cm.canonical_name
  FROM raw_split rs JOIN canonical_map cm ON cm.raw_name = rs.company_name
),
-- Domain resolved from the canonical registry, NOT from whichever variant
-- happened to be mentioned. Prefer the self-named variant's domain.
...
current_scored AS (
  SELECT ca.canonical_name, ca.industry_context, ca.country,
         NULL::text AS data_period, ca.mention_count, ca.combinations,
...
covered AS (SELECT DISTINCT industry_context, country FROM current_scored),
slice_latest_historical AS (
  SELECT industry_context, country, max(index_period) AS latest_period
  FROM rankings_historical GROUP BY industry_context, country
),
fallback AS (
  SELECT h.canonical_name, h.industry_context, h.country,
         h.index_period AS data_period, h.mention_count, h.combinations,
  ...
  JOIN slice_latest_historical sl
    ON sl.industry_context = h.industry_context AND sl.country = h.country AND sl.latest_period = h.index_period
  WHERE NOT EXISTS (SELECT 1 FROM covered c WHERE c.industry_context = h.industry_context AND c.country = h.country)
)
SELECT cs.canonical_name,
       COALESCE(cd.website_domain, '') AS website_domain,
       cs.industry_context, cs.country, cs.data_period, cs.mention_count,
       cs.combinations, cs.all_industry_combinations,
       cs.score_all, cs.score_chatgpt, cs.score_google, cs.score_perplexity
FROM current_scored cs
LEFT JOIN canonical_domains cd ON cd.ck = lower(cs.canonical_name)
UNION ALL
SELECT fb.canonical_name,
       COALESCE(cd.website_domain, '') AS website_domain,
       fb.industry_context, fb.country, fb.data_period, fb.mention_count,
       fb.combinations, fb.all_industry_combinations,
       fb.score_all, fb.score_chatgpt, fb.score_google, fb.score_perplexity
FROM fallback fb

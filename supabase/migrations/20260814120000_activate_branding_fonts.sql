...
-- Applied to production as migration version 20260814115333; this file was
...
--
-- Uploading a licensed corporate font puts the file on a public URL; that is a
-- licensing question for the client, not a technical one. Flagged in the admin.
...
begin
  ...
  if v_link.expires_at < now() then return jsonb_build_object('error','expired'); end if;
  ...
  em as (
    select distinct on (e.name, amm.market_code) e.name, amm.market_code, e.id
    from ent e
  ...
  return jsonb_build_object(
    'org', jsonb_build_object(
      ...
      'heading_font', v_branding.heading_font,
      'body_font', v_branding.body_font,
      'heading_font_url', v_branding.heading_font_url,
      'body_font_url', v_branding.body_font_url,
    ...
    'routes', coalesce((
      select jsonb_agg(jsonb_build_object(
        'market_code', r.market_code, 'tier', r.tier, 'channel', r.channel,
        'platform', r.platform, 'destination_url', r.destination_url,
        'write_url', r.write_url, 'rationale_stat', r.rationale_stat,
        'fit_note', r.fit_note, 'is_local', r.is_local,
        'audience_functions', r.audience_functions, 'audience_seniority', r.audience_seniority,
        'rank', r.rank, 'use_direct_link', r.use_direct_link,
        'entity_company_id', r.entity_company_id) order by r.market_code nulls last, r.rank)
      from public.activate_routes r
      where r.org_id = v_link.org_id and r.active), '[]'::jsonb),
    'highlights', coalesce((
      select jsonb_agg(jsonb_build_object(
      ...
      where h.org_id = v_link.org_id), '[]'::jsonb),
    'themes', coalesce((
      select jsonb_agg(jsonb_build_object(
        'market_code', t.market_code, 'theme', t.theme,
        'detail', t.detail, 'rank', t.rank) order by t.rank)
      from public.activate_market_themes t
      where t.org_id = v_link.org_id), '[]'::jsonb)
  );
end $$;

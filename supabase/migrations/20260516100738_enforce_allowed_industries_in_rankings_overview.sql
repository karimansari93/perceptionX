...
-- Applied to production as migration version 20260516100738; this file was
...
WITH raw_data AS (
  SELECT pr.ai_model, cp.industry_context, cp.location_context AS country,
         cp.prompt_theme, pr.detected_competitors
...
industry_stats AS (
  SELECT raw_data.industry_context, raw_data.country,
...
company_stats AS (
  SELECT raw_split.industry_context, raw_split.country, raw_split.company_name,
...
with_canonical AS (
  SELECT cs.company_name,
         ...
         cs.industry_context, cs.country, cs.mention_count, cs.combinations,
...
canonical_aggregated AS (
  SELECT with_canonical.canonical_name,
         ...
         with_canonical.industry_context, with_canonical.country,
...
scored AS (
  SELECT ca.canonical_name, ca.website_domain, ca.industry_context, ca.country,
         ...
         COALESCE(round(LEAST(100.0, (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c))
           / NULLIF((SELECT count(*)::numeric FROM unnest(ca.all_industry_combinations) c(c)), 0::numeric) * 100::numeric), 1), 0::numeric) AS score_all,
         COALESCE(round(LEAST(100.0, (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c) WHERE c.c ~~* '%gpt%')
           / NULLIF((SELECT count(*)::numeric FROM unnest(ca.all_industry_combinations) c(c) WHERE c.c ~~* '%gpt%'), 0::numeric) * 100::numeric), 1), 0::numeric) AS score_chatgpt,
         COALESCE(round(LEAST(100.0, (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c) WHERE c.c ~~* '%google%' OR c.c ~~* '%gemini%' OR c.c ~~* '%bard%')
           / NULLIF((SELECT count(*)::numeric FROM unnest(ca.all_industry_combinations) c(c) WHERE c.c ~~* '%google%' OR c.c ~~* '%gemini%' OR c.c ~~* '%bard%'), 0::numeric) * 100::numeric), 1), 0::numeric) AS score_google,
         COALESCE(round(LEAST(100.0, (SELECT count(*)::numeric FROM unnest(ca.combinations) c(c) WHERE c.c ~~* '%perplexity%')
           / NULLIF((SELECT count(*)::numeric FROM unnest(ca.all_industry_combinations) c(c) WHERE c.c ~~* '%perplexity%'), 0::numeric) * 100::numeric), 1), 0::numeric) AS score_perplexity
  ...
  WHERE cet.estimated_tier IS NOT NULL
    AND (cet.estimated_tier <> ALL (ARRAY['<50'::text, '50-499'::text, '500-4999'::text, 'unknown'::text]))

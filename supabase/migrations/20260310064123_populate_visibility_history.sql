...
-- Applied to production as migration version 20260310064123; this file was
-- back-filled afterwards and therefore post-dates the deployment.


-- Populate company_visibility_history for all index periods
-- using prompt_responses joined to confirmed_prompts
...
WITH period_mentions AS (
  SELECT
    unnest(string_to_array(pr.detected_competitors, ', ')) as canonical_name,
    cp.industry_context,
  ...
  GROUP BY
    unnest(string_to_array(pr.detected_competitors, ', ')),
    cp.industry_context,
    cp.location_context,
    pr.index_period
),
...
-- Compute rank and total per industry + country + period
ranked AS (
  SELECT
    canonical_name,
    industry_context,
    ...
    index_period,
    mention_count,
    -- Visibility score: log scale normalized 0-100
    round(
      least(100, (ln(mention_count + 1) / ln(50)) * 100)::numeric,
      1
    ) as visibility_score,
    rank() OVER (
...
SELECT
  ...
  round(
    (1 - (rank_position - 1.0) / nullif(total_in_industry - 1, 0)) * 100
  , 1) as percentile
FROM ranked

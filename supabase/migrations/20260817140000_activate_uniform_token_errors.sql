...
--
-- The token in an activate URL is the only credential the page has, and the
-- page it unlocks names the client. Until now the lookup answered three
-- different ways — 'not_found' for a token that never existed, 'revoked' and
-- 'expired' for one that did. The UI collapses all three to the same dead-link
-- screen (Activate.tsx), but the RPC response does not, so anyone holding a
-- candidate token could tell "never existed" from "existed, now closed" and
-- confirm a link had been real.
--
-- That distinction is worth nothing to a recipient and everything to a prober,
-- so it goes: every failure now returns 'not_found'. The revoked/expired tests
-- also move into the WHERE clause, which keeps the work identical on both
-- paths — a live token and a dead one are one index probe on the unique
-- activate_links.token index either way, rather than the dead path returning
-- measurably sooner.
--
-- What this deliberately does NOT try to hide: a *valid* token identifies its
-- client, to anyone holding it. Possession is the credential. The only thing
-- being closed is learning something without one.

------------------------------------------------------------------------------
-- Recipient: load the page config by token.
------------------------------------------------------------------------------
...
begin
  ...
  return jsonb_build_object(
    ...
    'entities', v_entities,
    -- The client's own job functions, commonest first. The step still accepts
    -- free text, so this is a shortcut list rather than a taxonomy.
...
------------------------------------------------------------------------------
-- Recipient: record a declaration or a platform click.
--
-- Two overloads of this exist in the database: the current 10-argument form
-- (with p_channels, which is what the client always calls — see
-- logActivateEvent in src/lib/activate/api.ts) and a stale 9-argument one left
-- behind by an earlier `create or replace` that added a parameter rather than
-- replacing in place. Both are reachable through PostgREST, so both are patched
-- here; patching only the live one would leave the old oracle answering.
--
-- The later validation errors ('invalid_event', 'rate_limited', …) stay
-- distinct on purpose. They are only reachable once the token has already
-- resolved, so they tell a caller nothing they did not know by holding a
-- working link.
------------------------------------------------------------------------------
...
begin
  -- Same uniform failure as activate_get_by_token: closed and never-existed
...
-- The stale 9-argument overload, same treatment.
create or replace function public.activate_log_event(
  p_token text, p_session_id uuid, p_event_type text,
  p_market_code text default null, p_entity_company_id uuid default null,
  p_platform text default null, p_tier integer default null,

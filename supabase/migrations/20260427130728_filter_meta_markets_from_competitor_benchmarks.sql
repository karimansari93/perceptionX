...
-- Applied to production as migration version 20260427130728; this file was
-- back-filled afterwards and therefore post-dates the deployment.

-- Refresh the view definition to exclude meta-markets like "Global (All Countries)"
-- which aren't real country benchmarks. Cohort-thin markets (Netherlands n=2,
-- Argentina n=3) stay in the view; the dashboard layer can decide whether to
-- show them based on cohort_size.
...
responses AS (
  ...
  WHERE cp.location_context IS NOT NULL
    -- Filter meta-markets: keep only real countries
    AND cp.location_context NOT ILIKE '%global%'
    AND cp.location_context NOT ILIKE '%all countries%'
    AND cp.location_context NOT ILIKE '%worldwide%'
),
response_citations AS (
  SELECT
    r.company_name, r.market, r.response_id, r.company_mentioned,
...
response_themes AS (
  SELECT r.company_name, r.market, r.response_id,
...
ranked AS (
  SELECT m.*,
    ...
    COUNT(*) FILTER (WHERE visibility_pct IS NOT NULL) OVER (PARTITION BY market) AS vis_cohort_size,
    RANK() OVER (PARTITION BY market ORDER BY sentiment_pct DESC NULLS LAST) AS sentiment_rank,
    COUNT(*) FILTER (WHERE sentiment_pct IS NOT NULL) OVER (PARTITION BY market) AS sent_cohort_size,
    RANK() OVER (PARTITION BY market ORDER BY relevance_pct DESC NULLS LAST) AS relevance_rank,
    COUNT(*) FILTER (WHERE relevance_pct IS NOT NULL) OVER (PARTITION BY market) AS rel_cohort_size
  FROM metrics_pct m
),
peer_stats AS (
  SELECT r1.company_name, r1.market,
...
SELECT
  r.company_name, r.market, r.responses, r.mentions,
  r.pos_themes, r.neg_themes, r.total_citations, r.valid_citations,
  r.visibility_pct, r.visibility_rank, r.vis_cohort_size,
  ...
  r.sentiment_pct, r.sentiment_rank, r.sent_cohort_size,
  ps.sentiment_peer_avg, ps.sentiment_peer_median,
  ROUND((r.sentiment_pct - ps.sentiment_peer_avg)::numeric, 1) AS sentiment_gap,
  r.relevance_pct, r.relevance_rank, r.rel_cohort_size,
  ps.relevance_peer_avg, ps.relevance_peer_median,

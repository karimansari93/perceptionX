...
-- Applied to production as migration version 20260603073723; this file was
-- back-filled afterwards and therefore post-dates the deployment.

-- Make the sentiment & relevance MVs cycle-aware. They previously bucketed by
-- date_trunc('month', tested_at); now they honor collection_cycle when set and
-- otherwise fall back to that exact tested_at month (byte-identical for every
-- untagged row). Atomic drop+recreate (nothing depends on these MVs); all
-- indexes and grants are restored in the same transaction.

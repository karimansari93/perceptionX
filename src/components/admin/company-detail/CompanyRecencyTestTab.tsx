  ...
  const runRecencyTest = async () => {
    ...
    try {
      ...
      const { data: responses, error: responsesError } = await supabase
        ...
        .in('prompt_type', ['experience', 'competitive', 'discovery', 'informational'])
          ...
          response.prompt_responses.forEach((pr: any) => {
            if (pr.citations) {
              ...
              citations.forEach((citation: any) => {
                const sourceType = ['experience', 'informational'].includes(response.prompt_type) ? 'sentiment' :
                                  ['competitive'].includes(response.prompt_type) ? 'competitive' :
                                  ['discovery'].includes(response.prompt_type) ? 'visibility' : 'competitive';
                allCitations.push({
                  ...citation,

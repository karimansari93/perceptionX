...
interface SimpleThematicAnalysisProps {
  responses: PromptResponse[];
  companyName: string;
}

export const SimpleThematicAnalysis = ({ responses, companyName }: SimpleThematicAnalysisProps) => {
  ...
  // Separate themes by sentiment and sort by frequency
  ...
  const getSentimentIcon = (sentiment: number) => {
    ...
    if (sentiment >= -0.3) return <Target className="w-4 h-4 text-yellow-600" />;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-green-600">{positiveThemes.length}</p>
            <p className="text-sm text-gray-600">Positive Themes</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-red-600">{negativeThemes.length}</p>
      ...
      {/* Theme Tabs */}
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'positive' | 'negative')}>
...
const ThemeCard = ({ theme, rank }: ThemeCardProps) => {
  ...
  const getFrequencyBadgeColor = (frequency: number) => {
    ...
    if (frequency >= 5) return 'bg-yellow-100 text-yellow-800';
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <Badge className={getFrequencyBadgeColor(theme.frequency)}>
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                ...
                <span className={`font-medium ${getSentimentColor(theme.sentiment)}`}>
                  {theme.sentiment > 0 ? '+' : ''}{Math.round(theme.sentiment * 100)}% sentiment
                </span>
              </div>
            </div>
